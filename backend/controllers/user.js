...
exports.addFriend = async (req, res) => {
  try {
    if (req.user.id !== req.params.id) {
      ...
      return res
        .status(400)
        .json({ message: "You can't send a request to youreself" });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (
        ...
        res.json({ mesage: "unfollow success" });
      } else {
        res.status(400).json({ mesage: "Already not following" });
      }
    } else {
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (
        ...
        res.json({ mesage: "unfriend request accepted" });
      } else {
        res
          .status(400)
          .json({ message: "This account not in your friend's list" });
      }
    } else {
      res.status(400).json({ message: "You can't unFriend yourself" });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (receiver.requests.includes(sender._id)) {
        ...
        res.json({ mesage: "delete request accepted" });
      } else {
